...
- 输入 `#` 触发快捷语补全。空 query 时先展示类别；输入类别名或别名可直接展开该类别；输入其他文本时按标题、说明、别名和正文做模糊匹配。快捷语配置见 [Startup Guide 的快捷语配置](StartupGuide.md#35-快捷语配置)。
...
- `Ctrl+Alt+T`：激活当前前台 Codex session 对应的 Windows Terminal 窗口。
...
Review Prompt 复用 Prompt Editor 的输入框能力，包括拖入路径、粘贴文本、粘贴图片、`#` / `$` / `@` 补全和右键模板。
