...
- `ExploreTo`、`Open`、`Edit` 保留为当前 UI 中的禁用占位，本阶段不接业务逻辑。
- 操作结果同步到 Footer 全局状态。

本阶段对齐 `mvp1.txt` 中“简单 list 展示和接入”的范围，只做查看、diff 和提交入口，不实现 stash、不实现 add/unstage、不实现内置 diff 预览、不做分支切换、push/pull、冲突解决或完整 Git 客户端能力。
...
- 当前 UI 上的 `ExploreTo`、`Open`、`Edit` 不纳入 MVP1-6 业务范围，继续 `IsEnabled="False"`，避免视觉上移除已有工作区动作入口。
...
- `OpenDiff` / `Commit` 执行后不自动刷新，除非后续人工验证认为体验明显需要。
...
- 本阶段只接通 `OpenDiff`、`Commit`、`Refresh`；`ExploreTo`、`Open`、`Edit` 保持禁用。
...
- [ ] **6-T03 按工作区 UI 接通工具栏状态**：保持当前 `OpenDiff / Commit / ExploreTo / Open / Edit / Refresh` 布局，只让 `OpenDiff`、`Commit`、`Refresh` 按 MVP 规则启用，`ExploreTo`、`Open`、`Edit` 继续禁用；验收点是视觉结构不偏离当前工作区 UI。
...
- [ ] **6-T06 错误状态与人工验收**：覆盖非 Git workspace、无变更、工具缺失、Git 命令失败等状态；验收点是 UI 不崩溃，Footer 有可理解状态。
...
- `ExploreTo`、`Open`、`Edit` 在本阶段保持禁用，不出现半可用入口。
