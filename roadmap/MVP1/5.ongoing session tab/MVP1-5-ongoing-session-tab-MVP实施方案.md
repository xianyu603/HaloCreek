...
- session 被 `BringToFront` 时，SessionService 先停止该 session 的 watch，再把内部状态置为 `Front`。
- 原 `Front` session 被切回后台时，SessionService 再恢复该 session 的 watch。

## 3. 模块边界

### 3.1 SessionService

SessionService 是 UI 面向的 session 编排层，聚合 tmux 后台运行能力和 Windows Terminal 前台呈现能力。

职责：

- 提供 `Launch`、`BringToFront`、`Exit`、`GetSessions`。
...
public sealed class SessionService
{
    ...
    public IReadOnlyList<OngoingSessionInfo> GetSessions(string? workspacePath);
...
`StartWatching` 和 `StopWatching` 的存在是为了让 SessionService 实现 `Front` 和 watch 的互斥：

- session 进入 `Front`：SessionService 先将内部状态设为 `Front`，再调用 `StopWatching(identifier)`。
- session 离开 `Front`：SessionService 将内部状态改为后台状态，再调用 `StartWatching(identifier)`。
...
- 将 TmuxService 返回的 attach/switch 指令输入或传递给前台终端。
...
- `Front`：SessionService 内部认为该 session 当前被前台 Windows Terminal attach。该状态由 SessionService 维护，不来自 TmuxService probe。
...
- `Front` 状态由 SessionService 根据 HaloCreek 内部操作路径维护。
- 当某 session 被 `BringToFront`，SessionService 停止监听该 session。
...
Prompt Editor 调用 SessionService：

```text
SessionService.Launch
  -> TmuxService.Launch
...
Ongoing Sessions 调用 SessionService：

```text
SessionService.BringToFront(sessionId)
...
MVP 不做 Win32 窗口句柄查找、强制抢焦点、多 WT 窗口管理或跨终端复用保证。
...
TmuxService.StateChanged(identifier, BackgroundRunning/BackgroundIdle/Unknown)
  -> SessionService 确认该 identifier 当前不是 Front
  -> 更新内部 session 表
  -> SessionsChanged
```

如果通知到达时该 session 已经是 `Front`，SessionService 忽略该通知。
...
`Refresh` 只刷新 SessionService 内部列表，不承诺重新扫描外部 tmux 世界。

## 9. 验证步骤

### 9.1 验证后台运行

1. 在 Prompt Editor 输入一个会保持交互的 prompt 并 Launch。
