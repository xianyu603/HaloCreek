...
- 用 TmuxService 返回的 attach/switch 指令发起前台呈现。
...
`ShowFront` 的语义是“把给定 terminal command 呈现到 HaloCreek 当前前台 terminal 入口”。MVP 中 TerminalService 可以在构造时生成一个应用实例级 window identity，例如 `halocreek-front-{shortAppInstanceId}`，或使用固定的单实例 identity，例如 `halocreek-front`。具体选择编码时按 Windows Terminal 的复用行为验证后确定，但该策略应留在 TerminalService 内。

`TerminalCommandSpec` 可以先保持简单，但应表达为结构化命令，而不是 UI 层或 SessionLifecycleService 拼接的裸字符串：
...
PlatformInfrastructure 只解释 `WindowIdentity` 在平台参数层面的含义，例如映射到 Windows Terminal 的 named window 参数；不解释该 identity 为什么存在，也不决定 HaloCreek 应该复用一个窗口还是多个窗口。
...
SessionLifecycleService.BringToFront(sessionId)
  ...
  -> TerminalService.ShowFront(command)
...
- [ ] **5-T02 UI 与 VM 接线到空实现**：在 Prompt Editor 内落地横向 ongoing sessions 列表，展示 `Title`、`State`，支持双击 `Bring to Front`、单项 `Exit`、空状态；删除独立 Ongoing Sessions tab；`PromptEditorViewModel` 订阅 `SessionsChanged`，并继续按当前 workspace 过滤 `GetOngoingSessions(...)` 结果。本任务不要求真实 session 行为测试，验收点是 UI 能显示空状态和空实现返回的数据结构，项目可构建。
