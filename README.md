...
HaloCreek 是一个面向个人 Agent 编程工作流的 Codex shell wrapper。它把 workspace、prompt、Codex session、历史记录、Git 变更、应用内 session 状态和人工审阅入口放到一个轻量桌面应用里，用来降低 Human in the loop 场景里的重复操作成本，当前主要面向 Windows + Codex CLI + psmux 的本地开发环境。
...
- 优先优化 Windows 本地 Codex CLI 工作流，并通过 psmux 管理后台 session。
